 ...
 *
 * $HeadURL: svn://svn.pspdev.org/psp/trunk/psplinkusb/psplink/shell.c $
...
static int modexec_cmd(int argc, char **argv, unsigned int *vRet)
{
	...
	int  len;
	int  retv;
	struct SceKernelLoadExecVSHParam le;
	...
	if(handlepath(g_context.currdir, file, path, TYPE_FILE, 1))
	{
		...
		retv = sceKernelLoadExecVSHMs2(path, &le);
		if(retv<0){
			SHELL_PRINT("sceKernelLoadExecVSHMs2: failed: %08x\n", retv);
		}else{
			SHELL_PRINT("sceKernelLoadExecVSHMs2 OK.\n");
		}
	}
	...
	if(argc > 1)
	{
		...
		while(argv[1][i]){
			if(argv[1][i] == '1'){
				...
				flags |= DEBUG_BP_HARDWARE_READ;
			}else if(argv[1][i] == 'w'){
				flags |= DEBUG_BP_HARDWARE_WRITE;
			}else if(argv[1][i] == 'a'){
				flags |= DEBUG_BP_HARDWARE_ACCESS;
			}
