...
#endif
	width = sizeof(g_font);
	total_chars = sizeof(g_font) / FONT_Y;

	pFont = SDL_CreateRGBSurface(SDL_SWSURFACE, width, FONT_Y, 32, rmask, gmask, bmask, amask);
	if(pFont)
	{
		int x, y;
		int i;
		...
		SDL_LockSurface(pFont);
		pixel = (uint32_t*) pFont->pixels;

		for(i = 0; i < sizeof(g_font); i += FONT_Y)
		{
			for(y = 0; y < FONT_Y; y++)
			{
				for(x = 0; x < FONT_X; x++)
				{
					...
					bit = g_font[i+y] & (128 >> x);
					if(bit)
					{
						pixel[(width * y) + x] = 0xFFFFFFFF;
					}
					else
					{
						pixel[(width * y) + x] = 0;
		...
		SDL_UnlockSurface(pFont);
	}
}

void put_char_xy(SDL_Surface *screen, int x, int y, unsigned char ch)
{
	SDL_Rect src;
	SDL_Rect dest;
	...
	SDL_BlitSurface(pFont, &src, screen, &dest);
}
